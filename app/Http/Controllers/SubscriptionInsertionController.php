...
class SubscriptionInsertionController extends Controller
{
    public function confirmsubscription(Request $request)
    {
        ...
        if($sesrangetran == false)
        {
                    ...
                    $output=array();
                    $output['status']='Error';
                    $output['message']='some error occurs try again later';
        ...
        else
        {
            ...
            $planamt = strip_tags($request->input('planamt')); 
             Session::put('planamts',$planamt);
            
            $subpackage= strip_tags($request->input('subpackage')); 
           
            Session::put('subpackagess',$subpackage);
            ...
            elseif($subpackage=="Premium")
                $subhrc=(250/$hrc_currentvalue);
            else if($subpackage=="Premium - Topup")
            {
              $subhrc=($planamt/$hrc_currentvalue);
            }
        
           Session::put('subhrcc',$subhrc);
            ...
            if(($current_hrc_balance>= $subhrc) && $subhrc>0)
            {
                ...
                $today_reff_subscription = 0;$total_reff_subscription =0;
				$sungene_sql = SunGene :: where('n_id',$userid)->select('n_today_reff_subscription','n_agroup_subscription','n_bgroup_subscription')->first();
				$n_agroup = $sungene_sql->n_agroup_subscription;
			    $n_bgroup = $sungene_sql->n_bgroup_subscription;
				$n_agroup_subscription = $n_agroup + $subhrc;
				$n_bgroup_subscription = $n_bgroup + $subhrc;
				
                $today_reff_subscription = $sungene_sql->n_today_reff_subscription;   
            
                $total_reff_subscription =$today_reff_subscription +$subhrc;
    			
                $sungene_sql = DB::table('sun_gene')
                    ->where('n_id',$sponsorid)
                    ->update(['n_today_reff_subscription' => $total_reff_subscription]);
...
/////////////Sponsor Infinity income qualified checking	
				...
				$sqlbcmaster_sql1 = DB::table('bc_master')
					->where('pn_id',$sponsorid)
					->select('c_infinity_income_qualified','c_position')
					->get();
				
				$sponsor_infinity_income_qualified ="N";                				 	 
				if($sqlbcmaster_sql1)
				{
					foreach($sqlbcmaster_sql1 as $row2)
					{
						$sponsor_infinity_income_qualified = $row2->c_infinity_income_qualified;
						$sponsor_position = $row2->c_position;
					}
				}
				...
				$user_position = "";$user_infinity_income_qualified ="N";                				 	 
				if($sqlbcmaster_sql)
				{
					foreach($sqlbcmaster_sql as $row2)
					{
						$user_infinity_income_qualified = $row2->c_infinity_income_qualified;
						$user_position = $row2->c_position;
					}
				}			
				
		if($sponsor_infinity_income_qualified=="N"){			
...
/////////////user Infinity income qualified checking	Second option  
            if(($subpackage=="Premium" || "Premium Plus") && $user_infinity_income_qualified=="N"){
...
/////////////user Infinity income qualified checking	Second option end 
					 ...
					 /////payout calculation Start ///////////////////	
    					...
    					$parentid = $userid;
    					$childid = $sponsorid;
    				    $infinity_income_qualified = $sponsor_infinity_income_qualified;
    					...
    					if($childid>0)
						{
							while ($childid >0 && $level<=10 )//        --- InnerLoop to connect uplines
							{
							   	...
							   	$basic_count=0; $premium=0; $premiumplus=0;$precentage=0;
                        ...
                        //---------------------------------------
								...
								$sqlbcmaster3 = DB::table('bc_master')
				...
				///Group A Group B updation
    					...
    					if($childid>0)
						{
							while ($childid >0)//        --- InnerLoop to connect uplines
							{
							    $n_agroup_subscription = 0;$n_bgroup_subscription = 0;
                                $sungene_sql2 = SunGene :: where('n_id',$childid)->select('n_agroup_subscription','n_bgroup_subscription')->first();
                        		$n_agroup_subscription = $sungene_sql2->n_agroup_subscription;
                        		$n_bgroup_subscription = $sungene_sql2->n_bgroup_subscription;
                        		$n_agroup_subscription = $n_agroup_subscription  + $subhrc;
                        		$n_bgroup_subscription = $n_bgroup_subscription + $subhrc;  
                        				
								if($position == "L") {
								    
								    
								    	$sqlGeneUp = DB::table('sun_gene')
								    	            ->where('n_id', $childid)
								    	            ->update(['n_agroup_subscription' => $n_agroup_subscription ]);
								}
								else if($position=="R") {
								    
								    	$sqlGeneUp = DB::table('sun_gene')
								    	            ->where('n_id', $childid)
								    	            ->update(['n_bgroup_subscription' => $n_bgroup_subscription ]);
                ...
                catch(\Exception $ex)
                {
                    ...
                    $output=array();
                    $output['status']='error';
                    $output['message']=' Server Error Occurs';
                    return response()->json($output) ; 
                    throw $ex;
                }
            ...
            else
            {
                $output=array();
                $output['status']='error';
                $output['message']='Insufficient balance in your HRC Wallet!';
                return response()->json($output) ; 
            }
